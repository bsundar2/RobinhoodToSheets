...
class RobinhoodApiData(Enum):
    ...
    DIVIDEND = RobinhoodDataType(name='dividend_rate', type=float)
    DIVIDEND_PER_PERIOD = RobinhoodDataType(name='total_dividend', type=float)
    PE_RATIO = RobinhoodDataType(name='pe_ratio', type=float)
    TOTAL_DIVIDEND_PAID = RobinhoodDataType(name='amount_paid_to_date', type=float)
